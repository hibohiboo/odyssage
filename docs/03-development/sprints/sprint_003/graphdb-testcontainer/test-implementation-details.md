...
### テスト数サマリー
- **graph-scene.spec.ts**: 12テスト（CRUD完全カバー）
- **graph-scenario.spec.ts**: 4テスト（基本操作）
- **graph-scene-batch.spec.ts**: 7テスト（一括操作）
- **合計**: 23テスト

### カバレッジ
- **正常系**: 作成・取得・更新・削除・一括更新
- **異常系**: バリデーション・パラメータエラー・リソース未存在
- **統合系**: API間整合性・副作用確認
- **性能系**: 大量データ処理（50件）
...
4. **既存機能維持**: 拡張より形式変換に焦点

### Testcontainer使用
1. **`await using`**: 自動リソース管理で安全性確保
2. **環境変数設定**: プロセス環境とenvオブジェクトの両方更新
3. **シンプルなAPI**: コールバック形式で使いやすく
4. **事前設定**: 各テストに必要なデータを適切に準備

### 横展戦略
1. **段階的変換**: 一度に1ファイルずつ変換
2. **機能保持**: テスト数や内容を変更せず形式のみ変更
3. **統一性**: 全ファイルで同じヘルパーとパターンを使用
4. **依存関係考慮**: バッチ操作など前提条件があるテストに注意
