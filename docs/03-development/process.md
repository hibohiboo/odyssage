...
## 🔧 API移行・リファクタリングプロセス（2025-08-12追加）

### RESTful命名統一プロジェクトから得た知見
...
```bash
# フロントエンド使用状況確認
apps/frontend/**/*.{ts,tsx,js,jsx,vue} で全文検索
- 使用あり → 段階的移行
- 未使用 → 迅速削除候補
...
```bash
# 1. OpenAPI仕様更新
docs/redocly/openapi/paths/new-api.yaml    # 新API仕様作成
docs/redocly/openapi/paths/old-api.yaml    # 旧API廃止警告追加  
docs/redocly/openapi/api.yaml              # 両パス追加

# 2. バックエンド実装
apps/backend/src/route/index.ts            # 新旧並行実装

# 3. 移行テスト作成・実行
apps/backend/test/integrations/             # 移行互換性テスト
bun run test                                # 全テスト通過確認
```

**Day 2: 迅速削除（記憶が新しいうちに）**
```bash
# 1. フロントエンド未使用確認
Grep検索で利用状況の最終確認

# 2. 旧API完全削除
apps/backend/src/route/index.ts            # 旧API実装削除
docs/redocly/openapi/paths/old-api.yaml    # 旧API仕様ファイル削除
docs/redocly/openapi/api.yaml              # 旧パス設定削除

# 3. テスト最適化
apps/backend/test/integrations/             # 移行テスト削除、新APIテストに統一
...
**定量的効果**:
```bash
# 第1弾実績: GET /api/scenario/{id} → GET /api/scenarios/{id}
- コード削除: 約200行（実装・仕様・テスト）
- テスト効率化: 9→5テスト、実行時間43%短縮
- ファイル削除: 1個（OpenAPI仕様ファイル）
```

**定性的効果**:
- **認知負荷軽減**: 開発者が考慮すべきAPIエンドポイント統一
- **保守性向上**: 重複コード・設定による保守負荷解消
- **開発効率向上**: 新規機能開発時の迷いの解消

#### **「記憶が新しいうちに削除」の重要性**

**コンテキスト維持効果**:
- **変更背景**: なぜこの移行が必要だったかの記憶鮮明
- **影響範囲**: どのファイル・設定に影響するかの把握済み
- **関連知識**: 実装詳細・テスト内容の理解維持

**作業効率向上**:
- **ファイル場所**: 修正対象ファイルの記憶が新しい
- **設定内容**: 変更した設定項目の記憶が鮮明  
- **テスト観点**: 何をテストすべきかの判断が正確

**品質向上**:
- **削除漏れ防止**: 関連ファイルの把握による完全削除
- **影響範囲確認**: 変更影響の正確な評価
- **ロールバック準備**: 必要に応じた復元手順の確立

#### **次弾移行への活用**

**テンプレート化**:
```bash
# 移行作業のテンプレート化により50%工数削減見込み
1. OpenAPI仕様更新パターン
2. バックエンド新旧並行実装パターン  
3. 移行テスト戦略パターン
4. 迅速削除チェックリスト
...
3. **指針ファイル更新**: 発見した問題と対策をプロジェクトのCLAUDE.mdに必ず反映
...
品質保証作業の実施内容は開発証跡ファイル（`docs/03-development/sprints/`）に記録すること：
...
### 設計・仕様書
- [[sprints]] - 開発証跡ファイル保存場所
- `docs/redocly/openapi/` - OpenAPI仕様書
- [[../02-architecture/database-design]] - データベース設計ドキュメント
...
#development #process #tdd #quality