...
```markdown
## ページレベル分割
- React.lazy() を使用したページコンポーネントの動的インポート
- ルートベースのコード分割と遅延ロード
- Suspense と ErrorBoundary によるロード状態管理

## フィーチャーレベル分割
- フィーチャー単位での獨立したバンドル生成
- 使用頻度に応じた優先度設計
- Tree-shaking と Dead code elimination の最適化
...
```markdown
## React最適化技法
- React.memo() を使用したコンポーネントメモ化
- useMemo() / useCallback() での重い計算のキャッシュ
- コンポーネントの再レンダリング最小化
