...
#### 10. BDDテスト実行でのタイムアウトエラー（2025-07-31 継続作業）
...
- **エラー詳細**:
  ...
  ```
- **根本原因分析**:
  1. **前提条件ステップが空実装**: `Given('シーン「村の酒場」が順序2で作成済みである')` が実際にシーンを作成していない
  2. **UIにシーンデータが表示されない**: フロントエンド側でモックの空配列(`mockScenes = []`)を使用
  3. **セレクタが見つからない**: 「村の酒場」テキストが存在しないため`text=村の酒場`クリックが失敗
- **TDD原則に基づく修正方針**:
  - テストファーストでBDDステップ定義から不足機能を特定・実装
  - 前提条件でのシーン作成機能を先に実装
  - フロントエンド側のシーンデータ取得・表示機能を後から実装

### BDDテスト修正TODOリスト

#### Phase A: BDDテスト前提条件の実装
- [ ] **A1**: シーン作成前提条件ステップの実装
  - [ ] A1-1: `Given('シーン「村の酒場」が順序2で作成済みである')` でGraphDB APIを呼び出しシーン実作成
  - [ ] A1-2: Neo4jヘルパーでシーン作成処理の実装
  - [ ] A1-3: 前提条件ステップでのエラーハンドリング実装
- [ ] **A2**: シーンデータ検証ステップの実装  
  - [ ] A2-1: `Then GraphDBにシーンデータが保存されている` の実装
  - [ ] A2-2: Neo4jクエリでシーン存在確認処理
  - [ ] A2-3: シナリオ-シーン関係性の検証処理

#### Phase B: フロントエンドシーンデータ取得機能
- [ ] **B0**: シーン一覧取得API実装（OpenAPI First開発手順）
  - [ ] B0-1: OpenAPI仕様書作成 - GET /api/graph-scenes/scenario/{scenarioId}
  - [ ] B0-2: 統合テスト作成（OpenAPI仕様に基づく）
  - [ ] B0-3: バックエンドAPI実装（テストを通すための最小実装）
  - [ ] B0-4: 統合テスト実行・成功確認
- [ ] **B1**: シーンデータ取得APIクライアント実装
  - [ ] B1-1: GraphDBシーン一覧取得Hook作成
