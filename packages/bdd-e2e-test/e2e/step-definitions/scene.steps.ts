...
When(
  'シーン概要を「プレイヤーたちが森の奥で発見する古い小屋。重要な手がかりが隠されている。」と入力する',
  async function (this) {
    const pageActions = new PageActions(this.page);
    const overview =
      'プレイヤーたちが森の奥で発見する古い小屋。重要な手がかりが隠されている。';
...
When('ユーザーが新しいシーン「最終決戦の場」を作成する', async function (this) {
  ...
  await pageActions.fillByTestId(
    'scene-overview-input',
    'シナリオのクライマックスとなる戦闘シーン',
  );
