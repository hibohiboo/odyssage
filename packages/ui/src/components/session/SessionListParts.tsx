 ...
 * ステータスによるフィルタリングタブ
 */
export const StatusTabs: React.FC<StatusTabsProps> = ({ activeStatus, onStatusChange }) => {
  return (
 ...
 * ステータスタブの個別ボタン
 */
const StatusTabButton: React.FC<StatusTabButtonProps> = ({ isActive, onClick, label }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`px-4 py-2 border-b-2 ${activeClass}`}
    >
      {label}
    </button>
  );
